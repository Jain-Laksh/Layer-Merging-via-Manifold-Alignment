...
# ---------------------------
# Optimal assignment helpers
# ---------------------------

def _linear_assignment(cost_matrix):
    """
    Solve linear assignment for cost_matrix.
    Attempt to use scipy.optimize.linear_sum_assignment, fall back to greedy if not available.

    Args:
        cost_matrix (np.ndarray): Shape (n_rows, n_cols).

    Returns:
        row_ind, col_ind: arrays of matched indices, same length.
    """
    try:
        from scipy.optimize import linear_sum_assignment
        row_ind, col_ind = linear_sum_assignment(cost_matrix)
        return np.asarray(row_ind, dtype=np.int32), np.asarray(col_ind, dtype=np.int32)
    except Exception:
        # Greedy fallback, not optimal, but safe
...
def _match_and_permute_rows(reference_np, target_np, normalize=True):
    """
    Match rows of target_np to rows of reference_np using assignment, return permuted target.
    If shapes differ, function will return target_np unchanged.

    Args:
        reference_np (np.ndarray): shape (out, in)
        target_np (np.ndarray): shape (out, in)
        normalize (bool): whether to L2-normalize rows before matching.

    Returns:
        np.ndarray: permuted target_np aligned to reference_np rows.
    """
    if reference_np.shape != target_np.shape:
        # shapes differ, cannot do row-wise 1-to-1 matching reliably
        return target_np

    # compute row vectors
    ref_rows = reference_np.reshape(reference_np.shape[0], -1)
    tgt_rows = target_np.reshape(target_np.shape[0], -1)

    # normalize rows if requested
    if normalize:
        ref_norms = np.linalg.norm(ref_rows, axis=1, keepdims=True) + 1e-12
        tgt_norms = np.linalg.norm(tgt_rows, axis=1, keepdims=True) + 1e-12
        ref_rows_n = ref_rows / ref_norms
        tgt_rows_n = tgt_rows / tgt_norms
    else:
        ref_rows_n = ref_rows
        tgt_rows_n = tgt_rows

    # cost matrix: pairwise squared l2
    # using broadcasting to compute efficient pairwise distances
    # cost[i,j] = ||ref_i - tgt_j||_2
    rr = np.sum(ref_rows_n * ref_rows_n, axis=1, keepdims=True)
    tt = np.sum(tgt_rows_n * tgt_rows_n, axis=1, keepdims=True).T
    cost = np.sqrt(np.maximum(rr + tt - 2 * (ref_rows_n @ tgt_rows_n.T), 0.0))

    row_ind, col_ind = _linear_assignment(cost)

    # Build permuted target so that permuted_target[row_ind[i]] == target[col_ind[i]]
    permuted = np.zeros_like(target_np)
    # initialize as copy to keep any unmatched rows if any
    permuted[:] = target_np[:]
    for r, c in zip(row_ind, col_ind):
        permuted[r] = target_np[c]
    return permuted

def layer_fusion(model, layer1_idx, layer2_idx, ratio_i, weight_types):
    """
    Fuses two specified layers of the model by aligning neurons with assignment,
    then blending their weights using the provided ratio.

    If perfect row-wise matching is not possible, falls back to a safe weighted average.
    ...
    # Extract parameter objects keyed by full name
    layer1_params = {
        name: param
    ...
    for name in layer2_params:
        try:
            print(f"{name}: {tuple(layer2_params[name].shape)}")
        except Exception:
            pass

    ratio_j = 1.0 - ratio_i

    # Iterate over requested weight types, align and fuse
    for weight_type in weight_types:
        name1 = f"model.layers.{layer1_idx}.{weight_type}"
        name2 = f"model.layers.{layer2_idx}.{weight_type}"

        w1 = layer1_params.get(name1, None)
        w2 = layer2_params.get(name2, None)

        if w1 is None or w2 is None:
            # some models use different naming, skip if not found
            continue

        # Move to CPU numpy for matching, use float32 to reduce memory usage
        w1_np = w1.detach().float().cpu().numpy()
        w2_np = w2.detach().float().cpu().numpy()

        try:
            # Attempt row-wise matching, only when shapes match
            if w1_np.shape == w2_np.shape and w1_np.shape[0] >= 1:
                # Permute target rows to align with reference w1
                w2_permuted = _match_and_permute_rows(w1_np, w2_np, normalize=True)
                fused_np = ratio_i * w1_np + ratio_j * w2_permuted
            else:
                # shapes differ, fallback to simple weighted average with broadcasting if possible
                fused_np = ratio_i * w1_np + ratio_j * w2_np
        except Exception as e:
            # If anything goes wrong in matching, fallback to safe average and print warning
            print(f"Warning: neuron matching failed for {weight_type}, falling back to simple average, error: {e}")
            try:
                fused_np = ratio_i * w1_np + ratio_j * w2_np
            except Exception:
                # Last resort, skip this weight
                print(f"Error fusing {weight_type}, skipping.")
                continue

        # Convert fused_np back to tensor and copy into parameter data in-place
        fused_tensor = torch.tensor(fused_np, device=w1.device, dtype=w1.dtype)
        # In-place update of parameter
        try:
            w1.data.copy_(fused_tensor)
        except Exception:
            # sometimes parameter views or shapes mismatch, set via state_dict fallback
            sd = model.state_dict()
            sd[name1] = fused_tensor
            model.load_state_dict(sd, strict=False)

    # Remove layer2 module from module list, keep order
    # This mirrors original logic, but note that removing modules will change layer indices for subsequent merges
...
def main():
    ...
    # safer base directory within your current project folder
    base_dir = os.path.join(os.getcwd(), f"outputs/{model_name}/fused_{args.num_layer}_layers")
